...
int hasPlayerLost(int turns, const char* word, const char* guesses) {
    char* display_word = wordDisplay(word, guesses);
    // Check if there are still underscores (incomplete word) and turns left
    if (turns == 0 && strchr(display_word, '_') != NULL) {
        return 1;  // Player has lost
    }
    return 0;  // Player hasn't lost
}
